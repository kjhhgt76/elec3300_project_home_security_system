...
void LCD_DrawChar ( uint16_t usC, uint16_t usP, uint8_t cChar )
{
	...
	for ( ucPage = 0; ucPage < HEIGHT_EN_CHAR; ucPage ++ )
	{
		...
		{
			if ( ucTemp & 0x01 )
				LCD_Write_Data ( 0xFFFF );//appear dot to white
			
			else
				LCD_Write_Data (  0x0000 );//not appear dot to black
...
//Task 3
void LCD_DrawCircle ( uint16_t usC, uint16_t usP, uint16_t SR, uint16_t LR, uint16_t usColor)
{	
	/*
	...
	// (x-h)^2/a^2 + (y-k)^2/b^2
	//a = LR, b = SR
	float a_square = LR * LR;
	float b_square = SR * SR;
	for (uint16_t x = 0; x <= (usC + LR); x++){
		for (uint16_t y = 0; y <= (usP + SR); y++){
			float cal = (x-usC)*(x-usC)/a_square + (y-usP)*(y-usP)/b_square;
			if (cal < 1.03 && cal > 0.95)
