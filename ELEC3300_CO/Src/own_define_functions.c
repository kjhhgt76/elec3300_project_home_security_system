			...
			if (re_entry > 2){
				...
				*photoNum += 1;
				...
				if(newbmp(photoPath) == 0){
					HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1,GPIO_PIN_SET); // blue off = photo taken
					break;
				}
			}
			HAL_Delay(1000);
	...
	if (f_mount(&photoFATFS,SDPath,1) == FR_OK){ // if sd card is successfully detected.
		if (f_open(&photoFILE, photoPath, FA_WRITE |FA_CREATE_ALWAYS) == FR_OK ){
			...
			if (f_write(&photoFILE, &bmp , sizeof(bmp), &numberofbytes) == FR_OK){
				...
				LCD_Cam_Gram(); //function in lcd.c to initialize lcd to show the photo
				for (i = 0; i < 240; i++)
				{
					for (j = 0; j < 320; j++)
					{
...
int CO_ppm(float* R0){
	...
	float co_sensor_volt = (float)(co_sensor_val/1024.0);
