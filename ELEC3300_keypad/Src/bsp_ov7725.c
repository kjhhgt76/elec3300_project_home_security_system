...
Reg_Info Sensor_Config[] =
{
  ...
  {HSTART,    0x3f}, /*start horizontal*/
	{HSIZE,     0x50}, /*measuring width of image*/
	{VSTRT,     0x03}, /*start vertical*/
	{VSIZE,     0x78}, /*measuring height of image*/
	{HREF,      0x00},
	{HOutSize,  0x50},	/*width of image*/
	{VOutSize,  0x78},	/*height of image*/
	{EXHCH,     0x00},
...
ErrorStatus Ov7725_Init(void)
{
	...
	if(Sensor_IDCode == OV7725_ID)
	{
		for( i = 0 ; i < OV7725_REG_NUM ; i++ )
		{
			...
			{                
				return ERROR; //return error if fail to write a byte
			}
		}
	}
	else
...
void ImagDisp(void) //Read and display image
{
	uint16_t i, j;
	...
	for(i = 0; i < 240; i++) //width
	{
		for(j = 0; j < 320; j++) //height
		{
			//Read a RGB565 pixel from FIFO
			READ_FIFO_PIXEL(Camera_Data);	
			//write the pixel to LCD
